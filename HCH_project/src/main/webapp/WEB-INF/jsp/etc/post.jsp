...
<div id="wrap">
		...
		<table class="post-content-1">			
			<tr>
				<td colspan="2">
					...
					<c:if test="${member.id==etc.registrant or admin!=null}">
						<button type="button" onClick="location.href='/etc/edit-etc?id=${etc.id}'" class="short-button-edit">수정</button>
						<button type="button" onClick="location.href='/etc/delete-etc?id=${etc.id}'" class="short-button-delete">삭제</button>
					</c:if>					
				</td>
			</tr>
			<tr>
				<td rowspan="2" width="195px"><img src="${pageContext.request.contextPath}/resources/image/${etc.filename}" width="195px" height="250px" class="post-content-1-main-image"/></td>
				<td>
					<table>
						<tr>
							<td class="post-content-1-title">${etc.name}</td>
							<td>
								...
								<c:if test="${member!=null}">
									<c:choose>
										<c:when test="${favorite==true}">
											<form action="release-favorite?id=${etc.id}" method="post">
										...
										<c:otherwise>
											<form action="add-favorite?id=${etc.id}" method="post">
			...
			<tr>
				<td colspan="2">
					...
					<br>
					<p class="comment-content">${etc.review}</p>
					<br>
					<p class="comment-regdate">${etc.registrant} | ${etc.regdate}에 게시함.</p>
				</td>
			</tr>
		</table>		
	</div>
	<!-- 댓글 -->
	<div>
		<form action="post-comment?id=${etc.id}" method="post">
			<input type="hidden" name="registrant" id="registrant" value="${member.id}"/>
			<input type="hidden" name="posting" id="posting" value="${etc.id}"/>
					...
					<tr style="border-top: solid 1px #000000;">
						...
						<td>
							<c:if test="${member.id==comment.registrant}">								
								<button type="button" onClick="location.href='/etc/post?id=${etc.id}&cid=${comment.id}&commentEdit=delete'" class="short-button-delete">삭제</button>
							</c:if>
						</td>	
					</tr>
